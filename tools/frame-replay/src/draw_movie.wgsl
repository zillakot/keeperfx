fn movie_sample(c: Command, pixel: vec2<u32>) -> u32 {
    ...
    let origin = bitcast<i32>(c.accumulator.y) * i32(c.origin.z) + bitcast<i32>(c.accumulator.x);
    let delta = i32(pixel.y * c.origin.z + pixel.x) - origin;
    let stride = c.origin.z * row_step;
    ...
    for (var row_copy = 0u; row_copy < select(1u, 2u, (mode & 2u) != 0u); row_copy++) {
        let local = delta - i32(row_copy * ((c.origin.z / 4u) * 4u));
        if local < 0 { continue; }
