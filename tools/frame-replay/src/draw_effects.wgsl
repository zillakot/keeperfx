// bases hold the lens map and fade rows, each biased by one; zero keeps that table
// inside the per-call source at the offset its header names.
struct LensView { extent: vec4<u32>, bases: vec4<u32> }
@group(0) @binding(2) var<uniform> view: LensView;
fn address(i: u32) -> u32 { return view.extent.z + (i / view.extent.x) * view.extent.y + i % view.extent.x; }
@group(0) @binding(0) var<storage, read_write> indices: array<u32>;
@group(0) @binding(1) var<storage, read> assets: array<u32>;
fn data(i: u32) -> u32 { return byte(view.extent.w + i); }
fn word(n: u32) -> u32 {
    let a = n * 4u;
    return le32(view.extent.w + a);
}
fn lens_map(i: u32) -> u32 {
    if view.bases.x != 0u { return byte(view.bases.x - 1u + i); }
    return data(word(12u) + i);
}
fn lens_fade(i: u32) -> u32 {
    if view.bases.y != 0u { return byte(view.bases.y - 1u + i); }
    return data(word(13u) + i);
}
...
fn pixel(i: u32) {
    ...
    if word(0u) == 0u {
        let a = i * 4u;
        let sx = lens_map(a) | (lens_map(a+1u) << 8u);
        let sy = lens_map(a+2u) | (lens_map(a+3u) << 8u);
        ...
        let shade = min((lens_map(a) + lens_map(b)) >> 3u, 32u);
        result = lens_fade(shade * 256u + source(y * word(3u) + x));
    } else {
        let ox = min((x * word(7u)) >> 16u, word(14u)-1u);
        let oy = min((y * word(8u)) >> 16u, word(15u)-1u);
        let overlay = lens_map(oy * word(14u) + ox);
