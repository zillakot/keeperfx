...
impl DrawRenderer {
    ...
    pub fn readback(&mut self, target: u64) -> Result<Vec<u8>> {
        ...
        let bytes = mapped.chunks_exact(4).map(|word| word[0]).collect();
        drop(mapped);
        staging.unmap();
...
fn pack_commands(
    ...
    for c in commands {
            ...
            c.kind <= GPOLY_SPAN && c.blend <= 2 && c.colour <= 255 && c.transparent <= OPAQUE,
...
fn bin_commands(words: &[u32], width: u32, height: u32, limit: usize) -> Result<Vec<u32>> {
    ...
    for (index, command) in words.chunks_exact(28).enumerate() {
