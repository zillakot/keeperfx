...
volume is concentrated in terrain, sprites, primitives and the minimap: general
triangles (modes 0–26), movies, transitions, built-in lenses, Lua lenses, DBC text
and huge bitmaps have zero arena/command counters in the oracle samples so far, so
this is parity evidence for the families the control sessions reach, not for all of
them.
...
Offscreen replay falls 0.58–0.73 ms in every measured cell, uncapped busy 1080p reaches
253/268 FPS, and the serialized exclusive GPU union rises 7% to 4.30 ms/frame, of which the
minimap pass is 1.01 ms.
...
3. **Sprite asset interning**, in parallel with 2: `arena_sprite_hits` is 0 against 39,267
   misses, about 475 KB/frame and the largest remaining per-frame upload.
...
Deferred: minimap residency ([PR #43](https://github.com/zillakot/keeperfx/pull/43)) is parked
pending a rebase and a rescope after the dispatch-extent slice. Remaining presenter cost — host
submission after replay, software index upload, late cursor restoration — sits behind the items
above and must preserve `presentation_cpu` at most 1.0 ms and simulation cadence in uncapped
