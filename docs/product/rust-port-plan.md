...
**Coverage slice S3, the minimap payload split, is delivered**
([PR #54](https://github.com/zillakot/keeperfx/pull/54),
[measurements](../performance-baselines.md#minimap-payload-split-measured-2026-09-15)):
segmenting the payload into resident base segments and per-table versioned style tables cuts
`arena_minimap_bytes` 559,427 → 214,002/213,996 per frame capped and → 46,164/43,135
uncapped, and `arena_bytes_uploaded` 31% capped and 45% uncapped, at offscreen busy 1080p
against the PR #50 reference. FPS and replay are inside matched-run variability in both cap
modes, so this lands as a parity-and-upload-volume change with no regression shown and no
...
coverage slices and the performance items measure in.

Deferred: remaining presenter cost — host submission after replay, software index upload,
late cursor restoration — sits behind the items above and must preserve `presentation_cpu` at most 1.0 ms
