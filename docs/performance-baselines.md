...
`852188bd01ba69fe2ee3c52142230411a8d0e82f416b9a6f3e8dcd66a7f6504a`, against the PR #50 head
...
| --- | ---: | ---: | ---: | ---: | ---: |
| dungeon-busy | 179 → 178 | 475,046.3 → 12,196.0 | 8.497 → 5.270 | 0.000 → 14.247 | 1,004,132.5 → 536,764.5 |
...
Both sessions pass their gate on both binaries: 81,001 and 50,399 verified batches on the branch, 0 failures, invalid frames, flagged shades and rejected commands, 0 arena overflows and 0 arena evictions, with the documented single `shadow_prior_divergence` event in `dungeon-busy` on both binaries. Arena capacity is
...
style table, bounded by 5 MiB of live arena size classes. That bound is counted in arena
source bytes, so the GPU cost is 5 MiB in the packed asset format and 20 MiB in the
expanded one, and it holds that retention at the validated maximum of 16 background
colours. Each entry also owns one host copy of its bytes, at most 5 MiB, which is separate
from the whole-source copies C, the bridge and the renderer already stage per command and
separate from the arena's own host image. Three Rust-side gauges report it —
`minimap_cache_class_bytes` (live GPU bytes), `minimap_cache_cpu_bytes` (the owning host
copies) and `minimap_cache_evictions` — and they are asserted in the drawing tests. Unlike
PR #43's equivalents they are **not** in the C counter ABI, so they do not appear in
`KFX_WGPU_DRAW_STATS` output: adding them there would touch the `DrawCounters` layout that
PR #55 is changing. Neither figure is process or driver memory.
