...
| Root-space binning misaligns view-local commands against the 16x16 tile grid, so a small command in a view at a non-multiple-of-16 origin enters more tiles than it did in view space | Measured as part of the PR 7 GPU cost; PR 8's terrain binning and PR 11's folds must repay it before the default switches |
| Creature shadows are the dominant serial boundary, not ordered sprites | ~38 per frame at busy 640x480 against ~3 ordered sprites, so "≈ 4 raster passes" needs the mask chain hoisted (PR 12) as well as PRs 8-11 |
