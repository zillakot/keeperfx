...
- **A pass window is not exclusive.** It runs from that pass's own begin stamp to its own end stamp, so it includes whatever the pass waited through, and two windows in flight together overlap. Their sum regularly exceeds the frame's wall clock and must never be presented as a decomposition of it. `gpu_frame_ns` is the frame's first pass begin to last pass end, unioned across its submissions; it is a real GPU window, so it bounds the per-pass windows from above and the presentation wall clock bounds it. `KFX_WGPU_GPU_TIMING=2` (`scripts/profile-game.py --serial-gpu-timing`) drains the queue after every timed submission, which removes the overlap and makes the per-pass windows exclusive — but it serialises the frame, so it is a diagnostic, not a performance baseline.
