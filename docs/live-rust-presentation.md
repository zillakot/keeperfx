...
- `gpu_shadow_commands`: committed creature shadows. `shadow_scratch_upload_bytes` is zero because the mask chain is GPU resident; `shadow_scratch_copy_bytes` records verification re-seeds, so it is nonzero only under verification when the resident chain diverges — 65,536 in the busy control sample, one re-seed for its single `shadow_prior_divergence` event. `shadow_scratch_readback_bytes` is zero unless `KFX_WGPU_DRAW_VERIFY` is set, which adds one blocking 256 KiB scratch read per shadow.
...
- `target_trig_geometry_bytes`, `target_trig_table_bytes`, `other_asset_upload_bytes`, `target_trig_table_hits`, `target_trig_table_misses`, `target_trig_asset_buffers`, and `shadow_pairs` attribute shadow uploads and reuse. The three byte counters sum to `gpu_asset_upload_bytes`. Arena miss causes, explicit forgets, capacity/live/retired/growth gauges and separately counted `preparer_buffers` / `preparer_buffer_bytes` are defined in the [counter reference](performance-baselines.md#shadow-asset-reuse-counters).
