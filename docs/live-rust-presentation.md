...
- `gpu_shadow_commands`: committed creature shadows. `shadow_scratch_upload_bytes` is zero because the mask chain is GPU resident; `shadow_scratch_copy_bytes` records verification re-seeds, so it is nonzero whenever the resident chain diverges — 65,536 in the busy control sample, one re-seed for its single `shadow_prior_divergence` event. `shadow_scratch_readback_bytes` is zero unless `KFX_WGPU_DRAW_VERIFY` is set, which adds one blocking 256 KiB scratch read per shadow.
